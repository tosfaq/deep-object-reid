...
class MetricLosses:
    ...
    def __call__(self, features, centers, labels, iteration, name='ml'):
        ...
        glob_push_plus_loss_val = self.glob_push_plus_loss(features, centers, labels)
        if self.writer is not None:
            self.writer.add_scalar('Loss/{}/global_push'.format(name), glob_push_plus_loss_val, iteration)

        total_loss = self.center_coeff * center_loss_val + self.glob_push_coeff * glob_push_plus_loss_val
