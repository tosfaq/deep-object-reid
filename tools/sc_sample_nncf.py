...
def parse_args():
    ...
    parser.add_argument('--load-weights', default=None, help='Load model to skip training step')
    args = parser.parse_args()
    return args

def sc_train_fp32(args, dataset, model_template, environment):
    ...
    # with open(osp.join("/tmp", "weights.pth"), "wb") as f:
...
def main(args):
    ...
    if not args.load_weights:
        output_model = sc_train_fp32(args, dataset, model_template, environment)
    else:
    ...
    output_nncf_model = ModelEntity(
        dataset,
        environment.get_model_configuration(),
        model_status=ModelStatus.NOT_READY)
    task.optimize(OptimizationType.NNCF, dataset, output_nncf_model)

    # with open(osp.join("/tmp", "weights_nncf.pth"), "wb") as f:
    #     f.write(output_nncf_model.get_data("weights.pth"))
